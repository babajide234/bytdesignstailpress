...
<?php if ( ! WC()->cart->is_empty() ) : ?>
	<div class=" flex flex-col gap-24 h-full">
		...
		<ul class=" w-full pt-[22px] flex-col justify-center items-center gap-3 flex woocommerce-mini-cart cart_list product_list_widget <?php echo esc_attr( $args['list_class'] ); ?>">
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<div class="w-full px-5 justify-between gap-4 items-center inline-flex">
		...
		<div class="w-[378px] mx-auto flex flex-col gap-4">
			<p class="woocommerce-mini-cart__total total flex justify-between items-center p-2.5 border rounded-lg w-full">
				<span class="subtotal-label"><?php esc_html_e( 'Subtotal:', 'woocommerce' ); ?></span>
				<?php echo WC()->cart->get_cart_subtotal(); ?>
			</p>
			<div class=" w-full flex flex-col justify-between items-center gap-4">
					<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="button wc-forward text-white text-xl font-normal font-dm-sans w-full p-2.5 bg-primary rounded-[10px] justify-center items-center gap-2.5 inline-flex">
						<?php esc_html_e( 'View Cart', 'woocommerce' ); ?>
					</a>
					<a href="<?php echo esc_url( wc_get_checkout_url() ); ?>" class="button checkout wc-forward text-primary border border-primary text-xl font-normal font-dm-sans w-full p-2.5 bg-white rounded-[10px] justify-center items-center gap-2.5 inline-flex">
